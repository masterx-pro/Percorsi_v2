...
class PercorsiApp(App):
    ...
    def get_distance_mode(self):
        try:
            spinner_text = self.root.ids.distance_mode.text
    ...
    def load_excel(self, selection):
        ...
        self.root.ids.lat_spinner.values = columns
        self.root.ids.lon_spinner.values = columns
        self.root.ids.date_spinner.values = col_options
        self.root.ids.operator_spinner.values = col_options
        self.root.ids.label_spinner.values = col_options
        
        # Auto-detect columns
        for col in columns:
            col_upper = col.upper()
            if 'LAT' in col_upper:
                self.root.ids.lat_spinner.text = col
            elif 'LON' in col_upper:
                self.root.ids.lon_spinner.text = col
            elif 'DATA' in col_upper or 'DATE' in col_upper:
                self.root.ids.date_spinner.text = col
            elif 'OPERATORE' in col_upper or 'OPERATOR' in col_upper:
                self.root.ids.operator_spinner.text = col
            elif 'NOME' in col_upper or 'NAME' in col_upper or 'LABEL' in col_upper:
                self.root.ids.label_spinner.text = col
        
        self.root.current = 'column_select'
    
    def start_excel_optimization(self):
        lat_col = self.root.ids.lat_spinner.text
        lon_col = self.root.ids.lon_spinner.text
        ...
        date_col = self.root.ids.date_spinner.text
        op_col = self.root.ids.operator_spinner.text
        label_col = self.root.ids.label_spinner.text
    ...
    def _update_manual_preview(self):
        ...
        self.root.ids.preview_label.text = f"Coordinate: {len(coords)}"
    ...
    def _start_optimization(self):
        ...
        self.root.ids.progress_status.text = "Preparazione..."
    ...
    def _set_status(self, text):
        self.root.ids.progress_status.text = text
    ...
    def on_start(self):
        Clock.schedule_once(lambda dt: self._refresh_routes_list(), 0.5)
    
    def _refresh_routes_list(self):
        routes_list = self.root.ids.routes_list
    ...
    def _show_route_detail(self, route):
        ...
        # Update UI
        self.root.ids.route_title.text = route['name']
        self.root.ids.route_summary.text = f"Distanza: {self._format_distance(route['total_distance'])} | Tappe: {route['stops_count']}"
        
        # Populate stops list
        stops_list = self.root.ids.stops_list
    ...
    def center_map(self):
        if self.current_route:
            self.root.ids.map_widget.set_view(self.current_route['coords'])
    ...
    def choose_export_folder(self):
        ...
        def select_folder(instance):
            ...
            self.root.ids.export_path_label.text = f"Cartella: {self.export_folder}"
            popup.dismiss()
